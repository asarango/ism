 ...
 */
class InformeAprendizajeTotal2 extends \yii\db\ActiveRecord {
    ...
    public function genera_reporte_alumno($alumno, $paralelo, $quimestre) {
        ...
        $sentenciasNxx = new SentenciasNotasDefinitivasAlumno($alumno, $periodoId, $paralelo);
...
//        $sentencias->procesarAreas($modelParalelo->course_id, $paralelo);
        ...
        if ($modelAlmunos > 1) {
            foreach ($modelAlmunos as $data) {
                $sentenciasNxx = new SentenciasNotasDefinitivasAlumno($data->id, $periodoId, $paralelo);
//
...
//        $html .= '<br><strong>' . $corresponde . '</strong>';
    ...
    public function genera_cuerpo_pdf_detalle_materias($alumno, $malla, $paralelo, $modelEscalasProyectos, $modelEscalasComportamiento, $minima, $modelEscalasApro) {
        ...
        $html .= '<td class="tamano10 conBorde centrarTexto">QF</td>';
        $html .= '<td class="tamano10 conBorde centrarTexto">REC1</td>';
        $html .= '<td class="tamano10 conBorde centrarTexto">REC2</td>';
        $html .= '<td class="tamano10 conBorde centrarTexto">SUP</td>';
        $html .= '<td class="tamano10 conBorde centrarTexto">REM</td>';
        $html .= '<td class="tamano10 conBorde centrarTexto">GRA</td>';
        $html .= '<td class="tamano10 conBorde centrarTexto">FINAL</td>';
    ...
    private function asignaturas($alumno, $malla, $minima, $paralelo) {
        ...
        $modelRindeSupletorio = ScholarisCursoImprimeLibreta::find()->where(['curso_id' => $modelParalelo->course_id])->one();
...
//
        ...
        foreach ($modelMallaArea as $area) {
            ...
            if ($area['se_imprime'] == true) {
                ...
                $notas = $sentenciasNxx->get_nota_area($area['id']);
...
//            $html .= $this->materias($area['id'], $alumno, $usuario, $minima, $paralelo, $modelPeriodo->codigo);
            ...
            foreach ($modelMaterias as $clase) {
                ...
                if ($clase['se_imprime'] == true) {
                    ...
                    $notas = $sentenciasNxx->get_nota_materia($clase['materia_id'], $modelGrupo->id);
                    ...
                    $html .= $notas['mejora_q1'] < $minima ? '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['mejora_q1'] . '</strong></td>' : '<td bgcolor="' . $colorSin . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['mejora_q1'] . '</strong></td>';
                    $html .= $notas['mejora_q2'] < $minima ? '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['mejora_q2'] . '</strong></td>' : '<td bgcolor="' . $colorSin . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['mejora_q2'] . '</strong></td>';
                    $html .= $notas['supletorio'] < $minima ? '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['supletorio'] . '</strong></td>' : '<td bgcolor="' . $colorSin . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['supletorio'] . '</strong></td>';
                    $html .= $notas['remedial'] < $minima ? '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['remedial'] . '</strong></td>' : '<td bgcolor="' . $colorSin . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['remedial'] . '</strong></td>';
                    $html .= $notas['gracia'] < $minima ? '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['gracia'] . '</strong></td>' : '<td bgcolor="' . $colorSin . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['remedial'] . '</strong></td>';

                    $html .= $notas['final_total'] < $minima ? '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['final_total'] . '</strong></td>' : '<td bgcolor="' . $colorSin . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $notas['final_total'] . '</strong></td>';


                   $observacion = $this->observacion_nota($notas['final_total']);
...
//        $html .= $this->aprovechamiento($alumno, $usuario, $modelBloque, $malla, $minima);
        ...
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde">'.$sentenciasNxx->q1.'</td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        ...
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde">'.$sentenciasNxx->q2.'</td>';
        $html .= '<td class="tamano8 conBorde">'.$sentenciasNxx->finalNormal.'</td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '<td class="tamano8 conBorde">'.$sentenciasNxx->notaFinalAprovechamiento.'</td>';
        $html .= '<td class="tamano8 conBorde"></td>';
        $html .= '</tr>';
        
        $html .= $this->proyectos($alumno, $usuario, $modelBloque, $malla, 'q2', $paralelo);
...
//    }
    ...
    public function proyectos($alumno, $usuario, $modelBloque, $malla, $quimestre, $paralelo) {
        $sentencias = new \backend\models\SentenciasRepLibreta2();
        
        $periodoId = Yii::$app->user->identity->periodo_id;
        
        $sentenciasNxx = new \backend\models\SentenciasNotasDefinitivasAlumno($alumno, $periodoId, $paralelo);
        ...
        $grupo = $this->recuperaIdMateria($alumno, $paralelo);
        
        
        if(isset($grupo['grupo_id'])){
            //$grupoId = $grupo['grupo_id'];
            $notasxx = $sentenciasNxx->get_nota_materia($grupo['idmateria'], $grupo['grupo_id']);
        ...
        if (isset($modelProyectos)) {            
            ...
            $p1 = $sentencias->homologaProyectos($notasxx['p1']);
            $p2 = $sentencias->homologaProyectos($notasxx['p2']);
            $p3 = $sentencias->homologaProyectos($notasxx['p3']);
            $pr1 = $sentencias->homologaProyectos($notasxx['pr1']);
            $pr180 = $sentencias->homologaProyectos($notasxx['pr180']);
            $ex1 = $sentencias->homologaProyectos($notasxx['ex1']);
            $ex120 = $sentencias->homologaProyectos($notasxx['ex120']);
            $q1 = $sentencias->homologaProyectos($notasxx['q1']);

            $p4 = $sentencias->homologaProyectos($notasxx['p4']);
            $p5 = $sentencias->homologaProyectos($notasxx['p5']);
            $p6 = $sentencias->homologaProyectos($notasxx['p6']);
            $pr2 = $sentencias->homologaProyectos($notasxx['pr2']);
            $pr280 = $sentencias->homologaProyectos($notasxx['pr280']);
            $ex2 = $sentencias->homologaProyectos($notasxx['ex2']);
            $ex220 = $sentencias->homologaProyectos($notasxx['ex220']);
            $q2 = $sentencias->homologaProyectos($notasxx['q2']);

            $final_ano_normal = $sentencias->homologaProyectos($notasxx['final_ano_normal']);
            ...
            echo 'No existe grupo_id para alumno: '.$alumno. ' del paralelo: '.$paralelo;
            die();
        }
    ...
    private function recuperaIdMateria($alumnoId, $paralelo){
        $con = Yii::$app->db;
        $query = "select 	c.idmateria, g.id as grupo_id
