 ...
 */
class MecIsmRemedial extends \yii\db\ActiveRecord {
...
//        $pie = $this->genera_pie_pdf();
        ...
        $html = $this->estilos();
        $html .= $this->genera_cuerpo('CUADRO DE REMEDIALES', 'final_total', 'remedial');
        $mpdf->WriteHTML($html);
        //$mpdf->addPage();            
        //$mpdf->SetFooter($pie);

        $mpdf->Output('MEC-Remediales' . "curso" . '.pdf', 'D');
        exit;
    }
...
//            $html .= '<div class="centrarTexto"><img src="imagenes/instituto/logo/sellolibreta.png" width="100px"></div>';
    ...
    private function procesa_asignaturas($quimestre, $extra) {
        ...
        foreach ($this->arrayMaterias as $mat) {
            ...
            $html .= '<td align="center" class="bordesolido tamano6" colspan="">ER</td>';
        ...
        foreach ($this->modelAlumnos as $alumno) {
            ...
            $revisa = $this->revisa_si_tiene_remedial($alumno['id'], $quimestre, $extra);
            ...
            if($revisa > 0){            
                ...
                $html .= $this->procesa_notas($alumno['id'], $quimestre, $extra);
    ...
    private function procesa_notas($alumnoId, $quimestre, $extra) {
        $sentenciasMec = new MecProcesaMaterias();
        ...
        foreach ($this->arrayMaterias as $materia) {
            $notas = $sentenciasMec->get_nota($materia['id'], 
                    $alumnoId, $this->tipoCalificacion, $this->paralelo, $this->usuario,
                    $this->periodoCodigo);
...
//            die();
        ...
        if ($this->tieneProyectos > 0) {
            ...
            $html .= '<td height="10" align="center" class="bordesolido tamano6">' . $notaP[$quimestre]['abreviatura'] . '</td>';
        }
        ///////////// FIN DE PROYECTOS ////////////////////
    ...
    /****
     * metodo de revision para ver si tiene remediales
     */
    private function revisa_si_tiene_remedial($alumnoId, $quimestre, $extra) {
        $sentenciasMec = new MecProcesaMaterias();
        $sentenciasNotas = new Notas();
