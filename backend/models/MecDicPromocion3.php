 ...
 */
class MecDicPromocion3 extends \yii\db\ActiveRecord {
...
where	c.curso_id = $cursoId
    ...
    private function genera_reporte_pdf() {
        ...
        $mpdf->showImageErrors = true;
            ...
            foreach ($this->modelAlumnos as $alumno){
                $html = $this->genera_cuerpo('CERTIFICADO DE PROMOCIÓN', 'final_total', $alumno);
                $mpdf->WriteHTML($html);
                $mpdf->addPage();
            }
    ...
    private function genera_cabecera() {
        ...
        $modelTituloDosDistrito = ScholarisParametrosOpciones::find()->where(['codigo' => 'tit2mec'])->one();
        $modelTituloTresDistrito = ScholarisParametrosOpciones::find()->where(['codigo' => 'tit3mec'])->one();
        ...
        isset($modelTituloDosDistrito->valor) ? $titulo2 = $modelTituloDosDistrito->valor : $titulo2 = 'Configurar en parámetros: parametro id = 10, codigo = tit2mec, Nombre=título dos de certificados mec, valor=nombre de coordinación';
        isset($modelTituloTresDistrito->valor) ? $titulo3 = $modelTituloTresDistrito->valor : $titulo3 = 'Configurar en parámetros: parametro id = 10, codigo = tit3mec, Nombre=título Tres de certificados mec, valor=nombre del distrito';
        ...
        $html .= '<td align="right" clas="otroColor" style="color: #013958"><strong>'.$titulo1.'</strong></td>';
        $html .= '</tr>';
        $html .= '</table>';
        ...
        $html .= $titulo2.'<br>';
        $html .= $titulo3.'<br><br>';
        $html .= '<p class="tamanoTitulo2">'.$modelParalelo->course->xInstitute->name.'</p>';
        $html .= '</td>';
...
//        $html .= '<hr>';
    ...
    private function genera_cuerpo($titulo, $quimestre, $alumno) {
        ...
        $modelTituloAmie = OpInstitute::findOne($institutoId);
        
        isset($modelTituloAmie->codigo_amie) ? $tituloAmie = $modelTituloAmie->codigo_amie : $tituloAmie = '';
        ...
        $html .= '.tamanoTitulo{font-size:20px;}';
        $html .= '.tamanoTitulo2{font-size:16px;}';
        ...
        $html .= '<tr>';
        $html .= '<td><br><br><br><br>';
        $html .= '<strong><p class="tamanoTitulo">'. $titulo . '</p></strong><br>';
        $html .= '<strong>AÑO LECTIVO '. $this->periodoCodigo . '</strong><br>';
        $html .= '<strong>JORNADA MATUTINA</strong><br>';
        $html .= '<strong>CÓDIGO AMIE: '.$tituloAmie.'</strong>';
        $html .= '</td>';
        $html .= '</tr>';
        $html .= '</table>';    
...
//                    
        ...
        if($this->tieneProyectos <> 0){
            ...
            $notaP = $procesosMec->get_proyectos($alumno['id'], $this->paralelo, 'q1');
            
            $notaPTransformada = $this->transforma_proyectos($notaP['q2']['abreviatura']);
    ...
    private function transforma_proyectos($nota){       
        ...
        isset($modelHomologacion->descripcion) ? $descripcion = $modelHomologacion->descripcion : $descripcion = '-';
        return $descripcion;
...
//         $html .= '<div class="tamano10">Dado y firmado en: '.$modelInstituto->store->company->partner->city.', '.$modelInstituto->store->company->partner->state->name.' '.$this->fecha_hoy().'</div>';
        ...
        $selloPromocionMec2 = ScholarisParametrosOpciones::find()->where(['codigo' => 'tit4mec'])->one();

        if(isset($selloPromocionMec2)){
