 ...
 */
class InformeNotasProfesorNac extends \yii\db\ActiveRecord {
    ...
    public function parcial($bloqueId, $claseId) {
        ...
        $mpdf->Output('Pacial Nac ' . "curso" . '.pdf', 'D');
        exit;
    }
...
//        $html .= '<td width="33%" align="center">{PAGENO}/{nbpg}</td>
    ...
    protected function html($claseId, $bloqueId) {
        ...
        $html .= '<th class="conborde" rowspan="2">#</th>';
        $html .= '<th class="conborde" colspan="2">CRITERIOS Y ACTIVIDADES</th>';
        ...
        foreach ($titulos as $tit) {
            ...
            $html .= '<th class="conborde" colspan="' . $tit['total'] . '"><center>' . $insumo . '</center></th>';
        }

        $html .= '<th class="conborde" rowspan="2" bgcolor="#ECE6F8">FINAL</th>';

        $html .= "</tr>";
        $html .= "<tr>";
        $html .= '<th class="conborde" colspan="2">Descripción y aspectos eavaluados</th>';
...
//        die();
        ...
        foreach ($tituloActividades as $tit) {
            if ($tit['title'] == 'PROMEDIO') {
                $html .= '<th class="conborde" bgcolor="#A6EBF7">' . $tit['title'] . '</th>';
            } else {
        ...
        foreach ($alumnos as $alumno) {
            ...
            foreach ($tituloActividades as $tit) {
                ...
                if ($tit['title'] == 'PROMEDIO') {
                    $html .= '<td class="conborde" align="center" bgcolor="#A6EBF7">' . $notasA . '</td>';
                } else {
...
//        }
    ...
    private function cuadroEstadistico($claseId, $bloqueId, $alumnos) {
        ...
        $sentencias = new SentenciasSqlFrontEnd();
        $i = 0;
        foreach ($alumnos as $alumno) {
            if ($alumno->inscription_state == 'M') {
                $i++;
