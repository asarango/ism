...
//        $html .= '<td align="center" width="33%"><img src="imagenes/instituto/logo/logo2.png" width="80"></td>';
    ...
    private function detalle_alumno($alumno, $paralelo, $conareas, $mallaId) {
        ...
        $html .= '.tamano16{font-size:16px; font-family: Arial;}';
        $html .= '.tamano10{font-size:12px; font-family: Arial;}';
        $html .= '.tamano8{font-size:8px; font-family: Arial;}';
        $html .= '</style>';
        ...
        $html .= 'De conformidad con lo prescrito en el Art. 197 del Reglamento General a la Ley Orgánica de '
                . 'Educación Intercultural y demás normativas vigentes, certifica que el / la estudiante</div> ' ;
        
         $html .=  '<p align="center" class="tamano16"><strong>'     . $alumno['last_name'] . ' ' . $alumno['first_name'] . ' ' . $alumno['middle_name'] . '</strong></p>';
         $html .= '<p align="justify" class="tamano10"><strong>del ' 
                . $modelParalelo->course->xTemplate->name . '  "' . $modelParalelo->name . '", </strong><br> CÓDIGO ESTUDIANTE N°' . $alumno['numero_identificacion'] 
...
//        $html .= '</table>';
    ...
    private function consecuencia($curso, $promedio, $totalBajos) {
        ...
        $modelFirmas = \backend\models\ScholarisFirmasReportes::find()->where(['template_id' => $modelCurso->x_template_id])->one();

        
        if($modelFirmas->secretaria_cargo == 'Secretaria' || 
           $modelFirmas->secretaria_cargo == 'SECRETARIA'){
        ...
        if($modelFirmas->principal_cargo == 'Rectora' || 
           $modelFirmas->principal_cargo == 'RECTORA' ||
           $modelFirmas->principal_cargo == 'Directora' ||
           $modelFirmas->principal_cargo == 'DIRECTORA'){
        ...
        $texto2 = 'Para certificar suscriben '.$preposicion.' '.$modelFirmas->principal_cargo.' y '.$preposicion2.' '.$modelFirmas->secretaria_cargo.'  del Plantel.';
        ...
        if ($modelRindeSup->rinde_supletorio == 1) {
            ...
            if ($promedio < $notaMinima->valor) {
                ...
                if($modelCursoNuevo){
                    ...
                    $html .= '<br><div class="tamano10">'.$texto2.'</div>';
                }
        ...
        $html .= '<div class="tamano10">Dado y firmado en: ' . $modelInstituto->store->company->partner->city . 
                ', ' . $modelInstituto->store->company->partner->state->name . ' ' . $fechaHoy . '</div>';
    ...
    private function detalle_materiasProyectos($curso, $alumno, $paralelo, $conareas) {
        ...
        $notas = $sentenciasNxx->get_notas_proyectos();
        ...
        foreach ($modelAreasNormal as $area) {
            ...
            $html .= '<tr>';
            if ($conareas == 'si') {
                $html .= '<td rowspan="' . count($modelMaterias) . '" class="bordesolido" colspan="2"><strong>' . $area->asignatura->nombre . '</strong></td>';
            }
...
//                $letras = $this->convertir_letras($nota);
                ...
                $html .= '<td align="center" class="bordesolido">' . $notaDesc . '</td>';
                $html .= '</tr>';
                $html .= '<tr>';
            }
        }
...
//                $html .= '<td class="bordesolido">' . $materia->asignatura->nombre . '</td>';
                ...
                $html .= '<td align="center" class="bordesolido">' . $compNota['descripcion'] . '</td>';
                $html .= '</tr>';
                $html .= '<tr>';
            }
        }
...
//                        ->orderBy([
//                         'ma.orden' => SORT_ASC,   
//                          'scholaris_mec_v2_malla_materia' =>SORT_ASC
//                        ])->all();
//
//
//        $html .= '<tr>';
...
//                $html .= '<td class="bordesolido">' . $materia->asignatura->nombre .$materia->orden. '</td>';
//            }else{
//                $promedia = '*';
//                $html .= '<td class="bordesolido"><strong>' .$promedia. $materia->area->asignatura->nombre . '</strong></td>';
//                $html .= '<td class="bordesolido"><strong>' . $materia->asignatura->nombre .$materia->orden. '</strong></td>';
//            }
//                       
//            
//
...
//    }
    ...
    private function detalle_materias($curso, $alumno, $paralelo, $conareas, $mallaId){
        ...
        $cont = 0;
        $menosdeMinimo = 0;
        $modelMaterias = $this->get_materias($mallaId);
        ...
        foreach ($modelMaterias as $materia) {
            ...
            if($materia['promedia'] == true){
                $promedia = '';
                $html .= '<td class="bordesolido">' .$promedia. $materia['area']. '</td>';
                $html .= '<td class="bordesolido">' . $materia['materia']. '</td>';
            }else{
                $promedia = '*';
                $html .= '<td class="bordesolido"><strong>' .$promedia. $materia['area'] . '</strong></td>';
                $html .= '<td class="bordesolido"><strong>' . $materia['materia']. '</strong></td>';
            }
...
//
    ...
    private function get_materias($mallaId){
        $con = Yii::$app->db;
        $query = "select 	a.nombre as area
		,m.nombre as materia
		,ma.orden, mm.orden 
		,mm.promedia 
                                    ,mm.id 
from 	scholaris_mec_v2_malla_materia mm 
		...
		inner join scholaris_mec_v2_asignatura m on m.id = mm.asignatura_id 
