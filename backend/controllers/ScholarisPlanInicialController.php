...
//                        'clase' => $clase
    ...
    public function actionDesagregar() {
        ...
        if ($model->load(Yii::$app->request->post())) {    
            $model->codigo_ambito = $modelAmbito->codigo;
            $model->save();
            return $this->redirect(['planificar',
                    'id' => $ambitoId,
    ...
    public function actionEliminarTodo(){
        ...
        $modelCurr = \backend\models\CurCurriculoDestreza::findOne($idCur);
        
        $model = \backend\models\ScholarisPlanInicial::find()
                ->where([
                    'clase_id' => $clase,
                    'quimestre_codigo' => $quimestre,
                    'codigo_destreza' => $modelCurr->codigo
                ])
                ->one();
        
        //$model->delete();
        return $this->render('eliminar-todo',[
                        'model' => $model,
                        'ambitoId' => $ambito,
                        'quimestre' => $quimestre,
                        'clase' => $clase
    ...
    public function actionEliminarTodoEjecuta(){
       
        $id = $_GET['id'];
        
        $this->eliminaCalificaciones($id);
        $model = \backend\models\ScholarisPlanInicial::findOne($id);
        
        $modelCurr = \backend\models\CurCurriculoDestreza::find()->where(['codigo' => $model->codigo_destreza])->one();
                
        
        $model->delete();
        return $this->redirect(['planificar',
                        'id' => $modelCurr->ambito_id,
                        'quimestre' => $model->quimestre_codigo,
                        'clase' => $model->clase_id
              ]);
    }
    
    private function eliminaCalificaciones($planId){
        $con = \Yii::$app->db;
        $query = "delete from scholaris_calificaciones_inicial where plan_id = $planId";
...
//
    ...
    public function actionMaterial(){
        ...
        return $this->redirect([
            '/scholaris-tarea-inicial/index1',
            'clase' => $clase,
            'quimestre' => $quimestre
        ]);
