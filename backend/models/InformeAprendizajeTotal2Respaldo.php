...
//                $notas = $sentencia->get_notas_por_materia($clase['clase_id'], $alumno);
                ...
                if($notas['final_total'] >= $minima){
                    $nf = $notas['final_total'];
                }elseif($notas['final_total'] < $minima){
                    $nf = $notas['final_ano_normal'];
                }
                
                $html .= '<td bgcolor="' . $colorBajo . '" class="tamano8 conBorde centrarTexto paddingTd"><strong>' . $nf . '</strong></td>' ;
                
                
                if($modelRindeSupletorio->rinde_supletorio){
                    $observacion = $this->observacion_nota($nf);  
