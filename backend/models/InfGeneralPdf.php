 ...
 */
class InfGeneralPdf extends \yii\db\ActiveRecord {
    ...
    public function __construct($paralelo, $alumno, $quimestre) {
        ...
        /** tota el total de parciales * */
        $this->total_parciales();
        ///////fin de total de parciales
        
        
        /**** valida parcial 3 si existe o no */
        
//        if($this->totalParciales > 4 && $this->quimestre = 'p3'){
//                $this->quimestre = $this->quimestre;
//        }else{
//            $this->quimestre = 'p4';
//        }
        ///////////////////////////////
    ...
    private function total_parciales() {
        ...
        $this->totalParciales = count($modelBloque);
    }
    ...
    private function titulo_reporte() {
        ...
        if ($this->totalParciales > 4) {
            ...
            switch ($this->quimestre) {
                ...
                case 'q1':
                    $this->tituloReporte = 'DEL QUIMESTRE 1 ';
                    break;
                
                case 'q2':
                    $this->tituloReporte = 'DEL QUIMESTRE 2 ';
                    break;
            }
        } else {
            switch ($this->quimestre) {
    ...
    private function genera_cuerpo() {
        ...
        $this->elimina_data_en_tabla();

        $this->guarda_data_table();
...
//                    $html .= '<td class="conBorde centrarTexto"  bgcolor="#eaeaea"><strong>' . $notaM . '</strong></td>';
                    ...
                    $sumaPromedioMaterias = $sumaPromedioMaterias + $notaM;
                    $cont++;
                }
            }
        }
...
//        $notaF = $this->calcula_promedio_final();
        $notaF = $sentencias->truncarNota($sumaPromedioMaterias / $cont, 2);
    ...
    private function guarda_data_table() {
        ...
        foreach ($this->modelAlumnos as $alumno) {
            $alumnoId = $alumno['id'];
            $alumnoNombre = $alumno['last_name'] . ' ' . $alumno['first_name'] . ' ' . $alumno['middle_name'];

            $promedios = $sentenciasNotasAlumnos->get_promedio_alumno($alumnoId, $this->paralelo, $this->usuario);
                        
            //NOTA PARA APROVECHAMIENTO
            ...
            if ($this->seccion != 'BACHILLERATO') {
                ...
                $proyQ1 = $proyectos->get_proyectos($alumnoId, $this->paralelo, $this->quimestre);

                //NOTA PARA PROYECTOS
                $notaProyectos = $proyQ1[$this->quimestre]['abreviatura'];
            ...
            //NOTA PARA APROVECHAMIENTO
            $notaComportamiento = $notaC[0][$this->quimestre];
            ...
            ////INSERTA REGISTRO
            $con = \Yii::$app->db;
            $query = "insert into scholaris_reporte_general (usuario, paralelo, alumno_id , estudiante, parcial_quimestre, promedio, proyectos, comportamiento)
                      values('$this->usuario', $this->paralelo, $alumnoId, '$alumnoNombre', '$this->quimestre', $notaFinal, '$notaProyectos', '$notaComportamiento');";

            $con->createCommand($query)->execute();
        }
    }
    
    private function elimina_data_en_tabla(){
        $con = \Yii::$app->db;
        $queryDelete = "delete from scholaris_reporte_general where paralelo = $this->paralelo and usuario = '$this->usuario';";
        $con->createCommand($queryDelete)->execute();
    }

    private function consulta_nota_general() {
        $con = \Yii::$app->db;
        
        $query = "select 	usuario, paralelo, alumno_id, estudiante, parcial_quimestre, promedio, proyectos, comportamiento 
                        from 	scholaris_reporte_general
                        where	paralelo = $this->paralelo
                                        and usuario = '$this->usuario' order by promedio desc, comportamiento asc;";
    ...
    private function detalle_alumnos() {
        ...
        $dataPromedios = $this->consulta_nota_general();

        foreach ($dataPromedios as $data){
        $i++;
        $html .= '<tr>';
        $html .= '<td class="conBorde">' . $i . '</td>';


        $html .= '<td class="conBorde">' . $data['estudiante'] . '</td>';

        $html .= '<td class="conBorde centrarTexto">' . $data['promedio'] . '</td>';
        if ($this->seccion != 'BACHILLERATO') {
            $html .= '<td class="conBorde centrarTexto">' . $data['proyectos'] . '</td>';
        }
        $html .= '<td class="conBorde centrarTexto">' . $data['comportamiento'] . '</td>';
...
//                if ($nota['bloqbuscar_notaue'] == $this->quimestre) {
//                    if($nota['nota'] < $this->notaMinima){
//                    $html .= '<td class="conBorde" align="center" style="background-color: red; color: black">' . $nota['nota'] . '</td>';
//                    }else{
//                        $html .= '<td class="conBorde" align="center">' . $nota['nota'] . '</td>';
...
//        die();
    ...
    private function calcula_promedio_materia($materiaId) {
        ...
        $nota = $sentenciasNotasAlumnos->get_promedio_materia($materiaId, $this->paralelo, $this->periodoId, $this->quimestre);
    ...
    private function calcula_promedio_final() {
        ...
        ///NOTA SOLO ESTA IMPLEMENTADO PARA CALIFICACION NORMAL

        $nota = $sentenciasNotasAlumnos->get_promedio_paralelo_parcial($this->paralelo, $this->usuario, $this->quimestre);
        return $nota;
    }
