 ...
 */
class InfSabanaFinalSuplePdfQ1P extends \yii\db\ActiveRecord {
    ...
    private function titulo_reporte() {
        if ($this->quimestre == 'q1') {
            ...
            $this->tituloReporte = 'FINAL CON SUPLETORIOS ';
        }
    }
            ...
            if ($materia['imprime']) {
                if ($materia['tipo_asignatura'] == 'area') {
                    ...
                    foreach ($this->modelAlumnos as $alumno){
                        ...
                        $grupoId = $this->busca_grupo_id($alumno['id'], $materia['asignatura_id']);
                        
                        $notaAlumno = $sentenciasNotasAlumnos->get_nota_materia($grupoId);
                        $notaAl = $notaAlumno[$this->quimestre];
                        
                        $sumaAl = $sumaAl + $notaAl;
                        $contAl++;
