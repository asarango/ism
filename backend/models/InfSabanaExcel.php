 ...
 */
class InfSabanaExcel extends \yii\db\ActiveRecord {
    ...
    public function genera_reporte($paralelo, $quimestre) {
        ...
        $modelUsaComportamiento = ScholarisParametrosOpciones::find()->where(['codigo' => 'comportamiento'])->one();
        if($modelUsaComportamiento->valor > 0){
    ...
    private function get_nota($alumnoId, $modelAreas, $modelParalelo, $modelPeriodo, $colspan, $usuario) {        
        ...
        foreach ($modelAreas as $area) {
           $notasArea = $sentenciasNxx->get_nota_area($area['id']);
            ...
            foreach ($modelMaterias as $materia) {
                $grupoId = $sentenciasAl->get_grupo($alumnoId, $materia['materia_id']);
                  ...
                  if($grupoId != false){
                      ...
                      $data .= '<td align="center"><strong>-</strong></td>';
                      $data .= '<td align="center"><strong>-</strong></td>';
                      $data .= '<td align="center"><strong>-</strong></td>';
                      $data .= '<td align="center"><strong>-</strong></td>';
                      $data .= '<td align="center"><strong>-</strong></td>';
            ...
            $modelCursoConfiguracion = ScholarisCursoImprimeLibreta::find()->where(['curso_id' => $modelParalelo->course_id])->one();
            if($modelCursoConfiguracion->tipo_proyectos != 'NOTIENE'){
                ...
                $data .= '<td align="center"><strong>NO APLICA</strong></td>';
                $data .= '<td align="center"><strong>NO APLICA</strong></td>';
            }
