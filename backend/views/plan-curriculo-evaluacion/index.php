...
/* @var $searchModel backend\models\PlanCurriculoEvaluacionSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Criterios de evaluación';
$pdfTitle = $this->title;
$this->params['breadcrumbs'][] = ['label' => 'Distribución', 'url' => ['plan-curriculo-distribucion/index']];
...
?>
<div class="plan-curriculo-evaluacion-index">
    ...
    <!--<div class="container">-->
    <p>
        <?= Html::a('Crear criterio de evaluacion', ['create', 'id' => $modelDistribucion->id], ['class' => 'btn btn-success']) ?>
    </p>
...
//            'distribucion_id',
            'codigo',
            'criterio_evaluacion:ntext',
            'orientacion_metodologica:ntext',
            ...
            /** INICIO BOTONES DE ACCION * */
                [
                    ...
                    'width' => '150px',
                    'vAlign' => 'middle',
                    'template' => '{view}{update}{objetivos}{destreza}',
                    'buttons' => [
                        'objetivos' => function($url, $model) {
                            return Html::a('<span class="glyphicon glyphicon-road"></span>', $url, [
                                        'title' => 'Objetivos_Generales', 'data-toggle' => 'tooltip', 'role' => 'modal-remote', 'data-pjax' => "0", 'class' => 'hand'
                            ]);
                        },
                        'destreza' => function($url, $model) {
                            return Html::a('<span class="glyphicon glyphicon-file"></span>', $url, [
                                        'title' => 'Destrezas_Evaluar', 'data-toggle' => 'tooltip', 'role' => 'modal-remote', 'data-pjax' => "0", 'class' => 'hand'
                            ]);
                        }
                    ],
                    'urlCreator' => function($action, $model, $key) {
                        if ($action === 'objetivos') {
                            return \yii\helpers\Url::to(['plan-curriculo-objetivos-generales/index1', 'id' => $key]);
                        } else if ($action === 'view') {
                            return \yii\helpers\Url::to(['view', 'id' => $key]);
                        } else if ($action === 'update') {
                            return \yii\helpers\Url::to(['update', 'id' => $key]);
                        } else if ($action === 'destreza') {
                            return \yii\helpers\Url::to(['plan-curriculo-destreza-evaluar/index1', 'id' => $key]);
                        } 
                    }
                ],
