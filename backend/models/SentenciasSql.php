...
//        die();
    ...
    public function criteriosNoAsignados($id_bloque,$id_actividad) {
        $con = Yii::$app->db;
       
        $query = "select 	vd.id, ic.nombre , icl.nombre_espanol ,ild.descripcion 
                 from 	planificacion_vertical_pai_descriptores vd
                inner join ism_criterio_descriptor_area maes on maes.id = vd.descriptor_id 
                ...
                inner join planificacion_bloques_unidad pbu on pbu.id = vd.plan_unidad_id
                inner join curriculo_mec_bloque cmb on cmb.id = pbu.curriculo_bloque_id
                inner join scholaris_bloque_actividad sba on sba.orden = cmb.code 
                where 	sba.id = $id_bloque
                and vd.id not in (select plan_vert_pai_descriptor_id 
                          from scholaris_actividad_descriptor sad 
                          where vd.id = sad.plan_vert_pai_descriptor_id  and actividad_id = $id_actividad  );
        ";
    //    echo '<pre>';
    //    print_r($query);
    //    die();
    ...
    public function criteriosAsignados($id_bloque,$id_actividad) 
    {
        $con = Yii::$app->db;
        $query = "select sad.id, ic.nombre , icl.nombre_espanol ,ild.descripcion 
                from 	planificacion_vertical_pai_descriptores vd
                inner join ism_criterio_descriptor_area maes on maes.id = vd.descriptor_id 
                inner join ism_criterio ic on ic.id = maes.id_criterio 
                inner join ism_criterio_literal icl on icl.id = maes.id_literal_criterio 
                ...
                inner join curriculo_mec_bloque cmb on cmb.id = pbu.curriculo_bloque_id
                inner join scholaris_bloque_actividad sba on sba.orden = cmb.code 
                inner join scholaris_actividad_descriptor sad on  vd.id = sad.plan_vert_pai_descriptor_id
                where sba.id = $id_bloque and sad.actividad_id =$id_actividad;";
