 ...
 */
class QuitoReportesMecNormalRemedialController extends Controller {
    /**
     * {@inheritdoc}
     */
...
//    }
    ...
    private function genera_cabecera_pdf($paralelo) {
        ...
        $html .= 'CUADRO FINAL CON REMEDIAL <br>';
...
//        $html .= 'AÑO LECTIVO ' . $modelPeriodo->nombre . '<br>JORNADA MATUTINA</div>';
        ...
        foreach ($modelAl as $al) {
            ...
            if ($total > 0) {
                ...
                for ($j = 0; $j < 9; $j++) {
                    ...
                    $notaSup = $sentencias2->truncarNota($sentenciasMec->get_nota_quimestre_v2($al['id'], $materiasN[$j]['id'], 'remedial', $paralelo)/$this->escala,2);
    ...
    private function toma_total_remediales($materiasN, $alumno, $paralelo, $notaMinima) {
        ...
        foreach ($materiasN as $mat) {
            $nota = $sentenciasMec->get_nota_quimestre_v2($alumno, $mat['id'], 'supletorio', $paralelo);
    ...
    private function toma_promedio($materiasN, $alumno, $paralelo, $notaMinima) {
        ...
        foreach ($materiasN as $mat) {
            ...
            $notaE = $sentenciasMec->get_nota_quimestre_v2($alumno, $mat['id'], 'remedial', $paralelo);
        ...
        foreach ($modelAl as $al) {
            ...
            if ($total > 0) {
                ...
                for ($j = 0; $j < count($materiasN); $j++) {
                    ...
                    $notaSup = $sentenciasMec->get_nota_quimestre_v2($al['id'], $materiasN[$j]['id'], 'remedial', $paralelo);
...
//                }
    ...
    private function get_estado($alumno, $materiasN, $paralelo) {
        ...
        for ($j = 0; $j < count($materiasN); $j++) {
//                
            $notaMat = $sentenciasMec->get_nota_quimestre_v2($alumno['id'], $materiasN[$j]['id'], 'final_con_mejora', $paralelo);
            $notaSup = $sentenciasMec->get_nota_quimestre_v2($alumno['id'], $materiasN[$j]['id'], 'supletorio', $paralelo);
            $notaFin = $sentenciasMec->get_nota_quimestre_v2($alumno['id'], $materiasN[$j]['id'], 'final_total', $paralelo);
...
//            $html .= '</tr>';
    ...
    private function devuelve_estado_gracia($nota, $countRemedial) {

        if ($countRemedial = 1) {
            $estado = 'GRACIA';
        } elseif ($countRemedial > 1) {
            $estado = 'PIERDE EL AÑO';
        } else {
