 ...
 */
class QuitoReportesMecNormalPromocionController extends Controller {
    /**
     * {@inheritdoc}
     */
...
//        $pie = $this->genera_pie_pdf();
    ...
    private function genera_cabecera_pdf($paralelo) {
        ...
        $html .= '</tr>';
        $html .= '<tr>';
        $html .= '<td align="center"><strong>CERTIFICADO DE PROMOCIÓN</strong></td>';
        $html .= '</tr>';
        $html .= '<tr>';
    ...
    private function detalle_alumno($alumno, $paralelo, $conareas) {
        ...
        $html .= 'De conformidad con lo prescrito en el Art. 197 del Reglamento General a la Ley Orgánica de '
                ...
                . $alumno['last_name'] . ' ' . $alumno['first_name'] . ' ' . $alumno['middle_name'].'</strong> del ' . $modelParalelo->course->xTemplate->name . ', PARALELO <strong>"' . $modelParalelo->name . '"</strong>';
...
//                $letras = $this->convertir_letras($nota);
    ...
    private function detalle_materiasComportam($curso, $alumno, $paralelo, $conareas) {
        ...
        foreach ($modelAreasNormal as $area) {
            ...
            $html .= '<tr>';
            if ($conareas == 'si') {
                $html .= '<td rowspan="' . count($modelMaterias) . '" class="bordesolido" colspan="2">' . $area->asignatura->nombre . '</td>';
            }
...
//                $html .= '<td class="bordesolido">' . $materia->asignatura->nombre . '</td>';
    ...
    private function detalle_materias($curso, $alumno, $paralelo, $conareas) {
        ...
        $modelNotaMinima = \backend\models\ScholarisParametrosOpciones::find()->where(['codigo' => 'notaminima'])->one();
        $notaMinima = $modelNotaMinima->valor / $escala;
