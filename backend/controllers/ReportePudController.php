...
//    }
    ...
    private function uno_datos($modelPud) {
        ...
        $html .= '<td class="conBorde" rowspan="2">' . $modelPud->fecha_inicio . '</td>';
        $html .= '<td class="conBorde colorEtiqueta" rowspan="2"><strong>FECHA DE FINALIZACIÓN:</strong></td>';
        $html .= '<td class="conBorde" rowspan="2">' . $modelPud->fecha_finalizacion . '</td>';
        $html .= '</tr>';

        $html .= '<tr>';
        $html .= '<td class="conBorde colorEtiqueta"><strong>NÚMERO DE PERÍODOS:</strong></td>';
        
        $cantPerio = $this->total_periodos($modelPud->id);
        
        
        $html .= '<td class="conBorde">'.$cantPerio.'</td>';
    ...
    private function total_periodos($pudId){
        $con = \Yii::$app->db;
        $query = "select 	sum(cantidad_periodos) as total
    ...
    private function dos_planificacion($modelPud) {
        ...
        foreach ($modelDestrezas as $destreza) {
            ...
            $html .= '<tr>';
            $html .= '<td class="conBorde colorEtiqueta" width="30%">'
                    . '<strong>DESTREZA CON CRITERIO DE DESEMPEÑO (' . $i . '): </strong><br>'
                    . '<strong>('.$destreza->cantidad_periodos.' PERIODOS)</strong>'
                    . '</td>';
...
//            die();
    ...
    private function tres_uno_actividades($modelPud) {
        ...
        $html .= '<td class="conBorde colorEtiqueta" width="50%" align="center">ACT.CALIFICADA</td>';
        ...
        foreach ($model as $activ) {
            if ($activ->insumo->tipo == 'F') {
                ...
                $html .= '<td class="conBorde" align="center">' . $activ->calificado . '</td>';
